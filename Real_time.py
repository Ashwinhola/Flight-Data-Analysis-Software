...
class RealTimePlott:   
    ...
    def plot_data_with_animation(self):
        def animate(frame):
            nonlocal self
            for param, data in self.plotted_param.items():
                eng_values = data['eng_val'][:frame + 1]
                timestamps = data['time_stamp'][:frame + 1]
                eng_values = [float(value) for value in eng_values]
        ...
        def play_animation():
            ...
            if not self.animation_running:
                ...
                self.animation = FuncAnimation(self.fig, animate, frames=len(self.stored_eng_val[list(self.stored_eng_val.keys())[0]]['eng_val']),
                                               interval=1000, repeat=False)
            else:
                messagebox.showwarning("Warning", "Animation is already running.")
        ...
        def pause_animation():
            ...
            if self.animation_running:
                ...
                messagebox.showwarning("Warning", "No animation is running.")

        self.play_button = tk.Button(self.your_frame, text="Play", command=play_animation)
        self.play_button.pack(side=tk.LEFT, padx=10)
        self.pause_button = tk.Button(self.your_frame, text="Pause", command=pause_animation, state=tk.DISABLED)
