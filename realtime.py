...
class RealTimePlot:   
    ...
    def Init_UI(self): 
        ...
        self.remove_button = tk.Button(self.root, text="Real Time plot", bg="#E3E3E3", command=lambda *args:self.update_plot)
        self.remove_button.place(x=300, y=300)
        self.remove_button.config(font=('Helvetica bold', 10))
    ...
    def check(self,name):
        ...
        directory=Path(r"C:\Users\Sanjana Kumari\OneDrive\Desktop\Intern")
    ...
    def update_plot(self, frame):
        ...
        if len(data) == 1:
            ...
            if selected_param in self.stored_eng_val:
                ...
                line, = self.ax.plot([], [], marker='o', linestyle='-', color='b', label='Data Points')
                self.ax.set_xlabel('Timestamp in seconds')
                ...
                for param, data in self.plotted_param.items():
                    ...
                    if eng_values and timestamps:
                        ...
                        def update(i):
                            line.set_data(timestamps[:i+1], eng_values[:i+1])
                            return line,

                        animation = FuncAnimation(self.fig, update, frames=len(timestamps), init_func=init,
                                                 blit=True, interval=1000)

                        playax = plt.axes([0.8, 0.01, 0.1, 0.05])
                        pauseax = plt.axes([0.9, 0.01, 0.1, 0.05])
                        button_play = Button(playax, 'Play', hovercolor='0.975')
                        button_pause = Button(pauseax, 'Pause', hovercolor='0.975')
